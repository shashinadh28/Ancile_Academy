        ...
        {UNIVERSITY_LOGOS.map(({ name, initials }, i) => (
          <div key={name} className={`transition-all duration-600 ease-out ${gridInView ? 'opacity-100 scale-100' : 'opacity-0 scale-75'}`} style={{ transitionDelay: `${i * 60}ms` }}>
            <div className="w-24 h-24 md:w-28 md:h-28 bg-white rounded-2xl border border-gray-100 flex flex-col items-center justify-center gap-1.5 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 group">
              <div className="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-gray-700 font-bold text-xs group-hover:bg-primary-100 group-hover:text-primary-700 transition-colors">{initials}</div>
