...
import { Mail, Phone, MapPin, ArrowUpRight, Star, Globe, MessageSquare, Camera, Briefcase } from 'lucide-react';
          ...
          {Object.entries(footerLinks).map(([heading, links], ci) => (
            <div key={heading} className={`transition-all duration-700 ease-out ${colsInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: `${(ci + 1) * 100}ms` }}>
              ...
              <ul className="space-y-2.5">
                {links.map((link) => (
                  <li key={link.label}><Link to={link.path} className="text-sm text-gray-500 hover:text-primary-400 transition-colors inline-flex items-center gap-1 group">{link.label}<ArrowUpRight size={10} className="opacity-0 group-hover:opacity-100 transition-opacity" /></Link></li>
                ))}
              </ul>
            </div>
          ))}
