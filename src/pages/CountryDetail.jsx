    ...
    <>
      ...
      <SectionWrapper>
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            <AnimateIn animation="fadeRight"><h2 className="text-2xl md:text-3xl font-bold text-warm-900 mb-4">Why Study in {country.name}?</h2></AnimateIn>
            <AnimateIn animation="fadeRight" delay={100}><p className="text-warm-500 leading-relaxed mb-8">{country.description}</p></AnimateIn>
          ...
          <AnimateIn animation="fadeLeft" delay={200}>
            <Card className="sticky top-28">
              <h3 className="text-lg font-bold text-warm-900 mb-4">Quick Inquiry</h3>
              <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); alert('Thanks! We will contact you soon.'); }}>
                <input placeholder="Your Name" className="w-full px-4 py-3 rounded-xl border border-warm-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500" />
                <input placeholder="Email" type="email" className="w-full px-4 py-3 rounded-xl border border-warm-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500" />
                <input placeholder="Phone" type="tel" className="w-full px-4 py-3 rounded-xl border border-warm-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500" />
      ...
      <SectionWrapper className="bg-warm-50">
        <div className="grid md:grid-cols-2 gap-6">
          <AnimateIn animation="fadeRight"><Card hover={false} className="h-full"><IconBox className="mb-5 bg-primary-100 text-primary-700"><BookOpen size={20} /></IconBox><h3 className="text-xl font-bold text-warm-900 mb-3">Education System</h3><p className="text-warm-500 leading-relaxed">{country.educationSystem}</p></Card></AnimateIn>
          <AnimateIn animation="fadeLeft" delay={200}><Card hover={false} className="h-full"><IconBox className="mb-5 bg-amber-100 text-amber-700"><Stamp size={20} /></IconBox><h3 className="text-xl font-bold text-warm-900 mb-3">Visa Information</h3><p className="text-warm-500 leading-relaxed">{country.visaInfo}</p></Card></AnimateIn>
        </div>
      </SectionWrapper>
        ...
        <div ref={uniRef} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {country.topUniversities.map((uni, i) => (
            <div key={uni} className={`transition-all duration-600 ease-out ${uniInView ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-6'}`} style={{ transitionDelay: `${i * 80}ms` }}>
              <div className="flex items-center gap-4 bg-white rounded-2xl border border-warm-100 p-5 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
                ...
                <span className="font-semibold text-warm-900 text-sm">{uni}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div ref={scholRef} className="max-w-2xl mx-auto space-y-3">
          {country.scholarships.map((s, i) => (
            <div key={s} className={`transition-all duration-600 ease-out ${scholInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`} style={{ transitionDelay: `${i * 100}ms` }}>
              <div className="flex items-center gap-3 bg-white rounded-xl border border-warm-100 p-4 hover:shadow-md transition-all"><CheckCircle size={18} className="text-primary-500 shrink-0" /><span className="font-medium text-warm-900 text-sm">{s}</span></div>
            </div>
          ))}
        </div>
      </SectionWrapper>
