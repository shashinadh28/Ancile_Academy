    ...
    <>
      ...
      <SectionWrapper className="bg-gray-50">
        <div className="grid md:grid-cols-2 gap-6">
          <AnimateIn animation="fadeRight"><Card hover={false} className="h-full"><IconBox className="mb-5 bg-primary-100 text-primary-700"><BookOpen size={20} /></IconBox><h3 className="text-xl font-bold text-gray-900 mb-3">Education System</h3><p className="text-gray-500 leading-relaxed">{country.educationSystem}</p></Card></AnimateIn>
          <AnimateIn animation="fadeLeft" delay={200}><Card hover={false} className="h-full"><IconBox className="mb-5 bg-amber-100 text-amber-700"><Stamp size={20} /></IconBox><h3 className="text-xl font-bold text-gray-900 mb-3">Visa Information</h3><p className="text-gray-500 leading-relaxed">{country.visaInfo}</p></Card></AnimateIn>
        </div>
      </SectionWrapper>
        ...
        <div ref={scholRef} className="max-w-2xl mx-auto space-y-3">
          {country.scholarships.map((s, i) => (
            <div key={s} className={`transition-all duration-600 ease-out ${scholInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`} style={{ transitionDelay: `${i * 100}ms` }}>
              <div className="flex items-center gap-3 bg-white rounded-xl border border-gray-100 p-4 hover:shadow-md transition-all"><CheckCircle size={18} className="text-primary-500 shrink-0" /><span className="font-medium text-gray-900 text-sm">{s}</span></div>
            </div>
          ))}
        </div>
      </SectionWrapper>
