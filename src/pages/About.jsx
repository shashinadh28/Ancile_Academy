...
const values = [
  { icon: Heart, title: 'Student First', description: 'Every decision is guided by what\'s best for our students\' academic and career growth.', color: 'bg-rose-100 text-rose-600' },
  { icon: Target, title: 'Excellence', description: 'We maintain the highest standards in counseling, documentation, and service delivery.', color: 'bg-violet-100 text-violet-600' },
  { icon: Users, title: 'Integrity', description: 'Transparent guidance with honest assessments — we never over-promise or mislead.', color: 'bg-blue-100 text-blue-600' },
  { icon: Globe, title: 'Global Perspective', description: 'Our diverse team brings worldwide experience and deep cultural understanding.', color: 'bg-emerald-100 text-emerald-600' },
];
    ...
    <>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            <AnimateIn animation="fadeRight">
              <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
                Transforming Lives Through <span className="text-primary-600">International Education</span>
              </h2>
            </AnimateIn>
          ...
          <div ref={statsRef} className="grid grid-cols-2 gap-4">
            {[
              ...
              <div
                key={stat.label}
                className={`transition-all duration-700 ease-out ${statsInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'} ${i % 2 === 1 ? 'mt-8' : ''}`}
      ...
      <SectionWrapper className="bg-gray-50">
        <AnimateIn animation="fadeUp">
          <SectionHeader title="What Drives Us Forward" align="center" />
        </AnimateIn>
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          <AnimateIn animation="fadeRight" delay={200}>
            <Card className="h-full" hover={false}>
              <IconBox className="mb-5 bg-primary-100 text-primary-600"><Target size={22} /></IconBox>
          ...
          <AnimateIn animation="fadeLeft" delay={300}>
            <Card className="h-full" hover={false}>
              <IconBox className="mb-5 bg-violet-100 text-violet-600"><Eye size={22} /></IconBox>
        ...
        <div ref={valuesRef} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {values.map(({ icon: Icon, title, description, color }, i) => (
            <div key={title} className={`transition-all duration-700 ease-out ${valuesInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} style={{ transitionDelay: `${i * 120}ms` }}>
              <div className="bg-white rounded-2xl p-6 border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-full text-center group">
        ...
        <div ref={teamRef} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {TEAM_MEMBERS.map((member, i) => (
            <div key={member.name} className={`transition-all duration-700 ease-out ${teamInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} style={{ transitionDelay: `${i * 120}ms` }}>
              <div className="bg-white rounded-2xl p-6 border border-gray-100 text-center group hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
