      ...
      <section className="gradient-hero relative overflow-hidden">
        ...
        <div className="container-custom relative z-10 py-24 md:py-32 px-4 sm:px-6 lg:px-8">
          ...
          <AnimateIn animation="fadeUp" delay={100}>
            <span className="inline-block px-3 py-1 rounded-full bg-primary-500/20 text-primary-300 text-xs font-semibold mb-4">
              {post.category}
            </span>
          </AnimateIn>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <article>
            {post.content.split('\n\n').map((block, i) => {
              if (block.startsWith('## ')) {
                return (
                  <AnimateIn key={i} animation="fadeUp">
                    <h2 className="text-2xl font-bold text-navy-900 mt-10 mb-4">{block.replace('## ', '')}</h2>
                  </AnimateIn>
                );
              }
              if (block.startsWith('### ')) {
                return (
                  <AnimateIn key={i} animation="fadeUp">
                    <h3 className="text-xl font-bold text-navy-900 mt-8 mb-3">{block.replace('### ', '')}</h3>
                  </AnimateIn>
                );
              }
              if (block.startsWith('- ')) {
              ...
              return (
                <AnimateIn key={i} animation="fadeUp">
                  <p className="text-gray-600 leading-relaxed mb-4">{block}</p>
                </AnimateIn>
              );
            })}
          </article>
            ...
            <div className="mt-16 pt-10 border-t border-gray-200">
              ...
              <div ref={relatedRef} className="grid md:grid-cols-2 gap-6">
                {relatedPosts.map((related, i) => (
                  <div
                    key={related.slug}
                    className={`transition-all duration-700 ease-out ${relatedInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                    style={{ transitionDelay: `${i * 150}ms` }}
                  >
                    <Link
                      to={`/blog/${related.slug}`}
                      className="group flex gap-4 bg-gray-50 rounded-xl p-4 hover:bg-primary-50 transition-colors"
                    >
                      <img
                        src={related.image}
                        alt={related.title}
                      ...
                      <div>
                        ...
                        <h4 className="font-bold text-navy-900 group-hover:text-primary-600 transition-colors line-clamp-2 mt-1">
                          {related.title}
                        </h4>
                        <span className="text-xs text-gray-400 mt-1 block">{related.readTime}</span>
                      </div>
                    </Link>
                  </div>
                ))}
